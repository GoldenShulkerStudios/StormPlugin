...
public class SetStormTimeCommand implements CommandExecutor {
    ...
    public SetStormTimeCommand(JavaPlugin plugin, StormListener stormListener) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player && sender.isOp()) {
            ...
            try {
                int currentTime = Integer.parseInt(args[2]);
                new BukkitRunnable() {
                    @Override
                    public void run() {
                        ...
                        if (configFile.exists()) {
                            ...
                            try (FileReader reader = new FileReader(configFile)) {
                                ...
                                config.setRemainingStormTime(currentTime);
